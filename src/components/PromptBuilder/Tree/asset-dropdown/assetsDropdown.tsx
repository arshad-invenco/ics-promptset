...
import {Assignment, Elements, TouchMapAreas} from "../../../../models/promptset.modal";
import {AREA, BG, ICON, IMAGE, INPUT, TEXT, TOUCH_MASK, TYPE, VIDEO,} from "../../../../constants/promptSetConstants";
...
export default function AssetsDropdown(props: AssetsDropdownProps) {
    ...
    const handleAsset = (asset: Asset) => {
        ...
        dispatch(addElementToAssignment({
            assignmentId: childState.id, newElement: MediaElement,
        }));
    ...
    const handleTouchMask = (touchMask: TouchMask | null) => {
        ...
        if (touchMask) {
            dispatch(addNewTouchMap({assignmentId: childState.id, newTouchMap: touchMask}))
        } else {
    ...
    function fetchAssets() {
        ...
        if (isSequoiaDevice("G7-100-8") && touchMask) setAssets((prevAssets) => [...prevAssets, "area"]); else if (isSequoiaDevice("G7-100") && !touchMask) setAssets((prevAssets) => [...prevAssets, "touchmask"]);
    ...
    function handleAdd(childState: Assignment, type: string) {
        ...
        if (type === TEXT) {
            ...
            dispatch(addElementToAssignment({
                assignmentId: childState.id, newElement: textElement,
            }));
        ...
        if (type === INPUT) {
            ...
            dispatch(addElementToAssignment({
                assignmentId: childState.id, newElement: inputElement,
            }));
