...
export default function InputSVG(props: PromptBuilderProps) {
    const { color, screenHeight, screenWidth } = props;
    const [elements, setElements] = useState<Elements[]>([]);

    // CONTEXT API
    const { activePromptEditorId, activeElementId, setActiveElementId, setActiveControlType } = useContext(promptSetContext);
    ...
    return (
        <svg id="svg" viewBox={`0 0 ${screenWidth} ${screenHeight}`}>
